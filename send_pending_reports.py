...
def load_local_env(env_path: str = ".env") -> None:
    ...
    # Let the .env file's last occurrence win so updated values in the file
    # override older duplicates from earlier lines.
...
CONFIG = {
    ...
    "from_email": os.environ.get("FROM_EMAIL", "noreply@edxso.com"),
...
def normalize_zepto_api_key(value: str) -> str:
    raw = (value or "").strip()
...
def build_zeptomail_payload(
    ...
    return {
        "from": {
            ...
            "name": CONFIG.get("from_name", "Team EDXSO"),
        },
        "to": [
            {
                "email_address": {
...
def send_email_with_zeptomail(
    ...
    headers = {
        ...
        "Authorization": f"Zoho-enczapikey {normalize_zepto_api_key(CONFIG['zepto_api_key'])}",
    }
